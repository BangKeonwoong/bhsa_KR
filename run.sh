...
PY_IN_VENV="$(find_venv_python || true)"
if [ -z "${PY_IN_VENV:-}" ]; then
...
# [ -z "${TF_UPDATE_ON_START:-}" ] && export TF_UPDATE_ON_START=0

repair_pip(){
  echo "[CTT Viewer] pip 복구 시도(ensurepip)"
  "$PY_IN_VENV" -m ensurepip --upgrade --default-pip || true
}

# Install dependencies (fast/robust with offline-aware behavior)
if [ -f "requirements.txt" ]; then
  echo "[CTT Viewer] 의존성 설치 확인(pip)"
  # Skip entirely if user requests
  if [ "${SKIP_PIP:-}" != "1" ]; then
    # Quick module check to avoid pip when already satisfied
    if "$PY_IN_VENV" - >/dev/null 2>&1 <<'PY'
...
PY
    then
      ...
      # prepare pip
      if ! "$PY_IN_VENV" -m pip --version >/dev/null 2>&1; then
        repair_pip
      fi
      if ! "$PY_IN_VENV" -m pip --version >/dev/null 2>&1; then
        ...
        if [ -x "$CURL_BIN" ]; then
          ...
          "$PY_IN_VENV" "$TMPPIP" || true
          rm -f "$TMPPIP"
        else
          echo "[CTT Viewer] curl 미존재로 pip 자동 설치를 생략합니다."
        fi
      fi
      # Speed up pip and prefer offline if wheels provided
      ...
      if ls data/wheels/*.whl >/dev/null 2>&1; then
        ...
        "$PY_IN_VENV" -m pip install --no-index --find-links data/wheels -r requirements.txt || true
      fi
      # If still missing, attempt online install (best-effort)
      if ! "$PY_IN_VENV" - >/dev/null 2>&1 <<'PY'
import importlib, sys
ok = True
for m in ("flask",):
...
PY
      then
        # Check quick network availability before trying online install
        if command -v curl >/dev/null 2>&1 && ! curl -sI --max-time 5 https://pypi.org/simple/ >/dev/null; then
          ...
          if [ "${UPGRADE_PIP:-}" = "1" ]; then
            "$PY_IN_VENV" -m pip install --upgrade "pip<25" || true
          fi
          "$PY_IN_VENV" -m pip install -r requirements.txt || true
        fi
      fi
    fi
  else
...
HOST=127.0.0.1
DISABLED_find_free_port(){
  local p
  for p in 5001 5000 8000 5050 5051 7000 7001; do
    if "$PY_IN_VENV" - "$p" <<'PY' >/dev/null 2>&1
import socket, sys
ok = True
s = socket.socket()
try:
...
export HOST PORT
URL="http://$HOST:$PORT/"
PORT_FILE="$SCRIPT_DIR/data/server_port.txt"; export PORT_FILE

echo "[CTT Viewer] 서버 시작: $URL"
"$PY_IN_VENV" app.py &
PID=$!

cleanup(){ echo; echo "[CTT Viewer] 서버 중지..."; kill "$PID" 2>/dev/null || true; }
...
if ! curl -s -o /dev/null "$URL" 2>/dev/null; then
  if [ -f "$PORT_FILE" ]; then
    ...
    [ -n "$ACTUAL_PORT" ] && PORT="$ACTUAL_PORT" && URL="http://$HOST:$PORT/"
  fi
fi
